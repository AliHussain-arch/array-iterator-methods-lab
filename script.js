...
inventorNames = inventors.map(function(inventor){
    inventorObject = {first: inventor.first,last: inventor.last };
...
inventorNamedAda = inventors.find(function(inventor){
    if(inventor.first === 'Ada') return true;
})
...
firstLast = people.map(function(people){
    lastName = people.split(", ");
    return lastName[1] +" "+lastName[0]
});
...
isAdultPresent = devs.some(function(person){
...
isEveryone19OrOlder = devs.every(function(person){
    return person.year - 2000 >= 18 ;
});
...
commentById = comments.find(function(user){
    return user.id === 823423;
});
...
idx = comments.findIndex(function(user){
    return user.id === 123523;
})
